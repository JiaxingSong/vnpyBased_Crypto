...
class KkStrategy(CtaTemplate):
    ...
    # 策略参数
    kkLength = 14           # 计算通道中值的窗口数
    kkDev = 1.6             # 计算通道宽度的偏差
    trailingPrcnt = 5     # 移动止损
    
    fixedSize = 1                  # 每次交易的数量
    maxLength = max(kkLength,0)      # 多个指标所需要的最大缓存天数
    initDays = maxLength           # 初始化数据所用的天数
    # 策略变量
    kkUp = 0                            # KK通道上轨
    kkDown = 0                          # KK通道下轨
    ...
    def __init__(self, ctaEngine, setting):
        ...
        self.bg = BarGenerator(self.onBar, 1, self.onFiveBar)     # 创建K线合成器对象
        self.am = ArrayManager(self.maxLength+1)
